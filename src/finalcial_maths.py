...
def prompt_values(userChoice):
	...
	if userChoice == 'Compound Interest':
		...
		try:
			initialDeposit = float(raw_input("\n{} Initial deposit: ".format(whiteSpace)))
			interestRate = float(raw_input("{} Annual interest rate (percent): ".format(whiteSpace)))
			timesCompounded = float(raw_input("{} Number of times the interest is compounded: ".format(whiteSpace)))
			time = float(raw_input("{} Number of years the money is invested for: ".format(whiteSpace)))
		except ValueError:
			print invalidInput
		...
		try:
			loanAmount = float(raw_input("\n{} Loan amount: ".format(whiteSpace)))
			interestRate = float(raw_input("{} Interest rate (percentage): ".format(whiteSpace)))
			time = float(raw_input("{} Duration: ".format(whiteSpace)))
		except ValueError:
			print invalidInput
			prompt_values(userChoice)
		except KeyboardInterrupt:
